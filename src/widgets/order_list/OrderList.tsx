...
import { GET_ORDER, GetOrderResponseQueries } from './query';
import CustomSpin from '../spin/CustomSpin';
import { Order } from 'src/shared/orders.types';
  ...
  const { loading: orderLoading } = useQuery<GetOrderResponseQueries>(GET_ORDER, {
    onCompleted: (data) => {
      if (data) {
        ...
        // const _orders = data.orders.getMany.data;
        // console.log('Orders loading success= ', JSON.stringify(_orders));
      }
    },
    onError: (error) => {
