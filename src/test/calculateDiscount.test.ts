...
describe('AccountService', () => {
  ...
  describe('Calculate Discount', () => {
    ...
    it('should apply only base discount when no category discount exists', async () => {
      service.setUserTypeDiscount(UserType.standart, 0.2);
      service.setCategoryDiscount(Category.food, UserType.standart, 0);

      const discount = service.calculateTotalDiscount(testUser.id, { ...testProduct, category: Category.food });
      expect(discount).toBe(0.2);
    });

    it('should apply only category discount when no base discount exists', async () => {
      service.setUserTypeDiscount(UserType.standart, 0);
      service.setCategoryDiscount(Category.car, UserType.standart, 0.3);

      const discount = service.calculateTotalDiscount(testUser.id, { ...testProduct, category: Category.car });
      expect(discount).toBe(0.3);
    });

    it('should return original price when no discounts apply', async () => {
      const totalDiscount = service.calculateTotalDiscount(generateUUID(), {
        ...testProduct,
